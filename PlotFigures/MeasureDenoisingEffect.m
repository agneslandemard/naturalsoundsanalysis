%% Compare raw and denoised responses(Fig S2)
%% B : exemple slice before/after removing out-of-cortex components
...
DiffNatMM_before = snm(B0.Sreco(D.param.snd.idxSound(:,1),:)-B0.Sreco(D.param.snd.idxSound(:,end),:),1);
DiffNatMM_before = Pixs2Mat(DiffNatMM_before(D.si == hemi)',X.param.msk);

DiffNatMM_after = snm(B.Sreco(D.param.snd.idxSound(:,1),:)-B.Sreco(D.param.snd.idxSound(:,end),:),1);
...
%% C-E: evaluate denoising part 2 (DSS...)
...
%% E : correlation original/denoised by nb of components DSS
