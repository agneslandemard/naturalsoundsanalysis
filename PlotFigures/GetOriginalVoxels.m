...
% load denoised data
load([analysis_path experiment '_' version '/D_denoised_K' num2str(n_comps) '.mat'],'D_denoised','si');
load([analysis_path experiment '_' version '/D_norm.mat'],'hemis');

% example for one hemisphere
hemi = 1;
Dc = permute(D_denoised(:,:,si == hemi,:),[3 1 2 4]); 
s = size(Dc);
