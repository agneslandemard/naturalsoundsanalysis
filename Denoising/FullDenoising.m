...
version_name = [experiment '_myVersion_name'] ;
...
for j = 1 : n_hemis
    ...
    for ii = 1 : n_slices
        ...
        load(fullfile(sliceList(ii).folder,sliceList(ii).name),'Din','Dout');
...
for run = 1:n_runs
    ...
    fprintf('Whitening\n'); drawnow;
    D_white = cell(1, n_hemis);
    for j = 1:n_hemis
        
        [n_tps, n_train_stims, n_reps, n_voxels] = size(D_norm{j}(:,~leaveout,:,:));
        
        D_white{j} = nan([n_tps*n_train_stims, n_reps, n_voxels]);
        
        for i = 1:n_reps
            
            n_slices = max(slice_refs{j});
            
            for ii = 1:n_slices
                
                vox_in_slice = find(slice_refs{j}==ii);
                
                % select one rep and one slice
                % concatenate time and stimulus dimension
                Z = reshape(D_norm{j}(:,~leaveout,i,vox_in_slice), [n_tps*n_train_stims, length(vox_in_slice)]);
    ...
    save([save_path '/Run_' num2str(run) '_components.mat'],'U_dss','leaveout')
...
if exist('finalK','var')
    ...
    save([save_path '/D_denoised_K' num2str(K) '.mat'],'D_denoised','si','-v7.3');
...
%% To cross validate nb of components
...
saveas(gcf,[save_path 'CV_NbComponents.png'])
