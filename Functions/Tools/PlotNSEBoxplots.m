function [corr_pvals_indiv,corr_pvals_bigCats] = PlotNSEBoxplots(D,sbj,SoundType,method)
...
ratioall = cell(length(sbj.Subjects));
ratioctrl = cell(length(sbj.Subjects));
xpos = @(h,s) s+(h-1)*(length(SoundType)+1);

m = 0;
pvals_allCats=nan(length(sbj.Subjects),length(SoundType),length(SoundType));
pvals_indiv=nan(length(sbj.Subjects),length(SoundType));
pvals_bigCats=nan(length(sbj.Subjects),2);

for f = 1 : length(sbj.Subjects)
    AvResp = snm(D.(sbj.Subjects{f})(D.param.snd.idxSound(:,[1 cond]),:,:),[1 3]);
    
    X_orig1 = snm(D.(sbj.Subjects{f})(D.param.snd.idxSound(:,1),:,1),3);
    X_mm1 = snm(D.(sbj.Subjects{f})(D.param.snd.idxSound(:,cond),:,1),3);
    
    X_orig2 = snm(D.(sbj.Subjects{f})(D.param.snd.idxSound(:,1),:,2),3);
    X_mm2 = snm(D.(sbj.Subjects{f})(D.param.snd.idxSound(:,cond),:,2),3);
    ...
    for sd = 1:length(SoundType)
     ...
     if any(~isnan(ratioall{f}(SelectSounds('Ferrets',D.param)))) && any(~isnan(ratioall{f}(SelectSounds('SpeechMusic',D.param))))
        pvals_bigCats(f,1) = ranksum(ratioall{f}(SelectSounds('Ferrets',D.param)),ratioall{f}(SelectSounds('SpeechMusic',D.param)));
     end
...
[~,~,corr_pvals_bigCats] = fdr_bh(pvals_bigCats(:));
corr_pvals_bigCats = reshape(corr_pvals_bigCats,size(pvals_bigCats));

[~,~,corr_pvals_allCats] = fdr_bh(pvals_allCats(:));
...
[~,~,corr_pvals_indiv] = fdr_bh(pvals_indiv(:));
...
title(name)
xticks(xpos(1:length(sbj.Subjects),2))
